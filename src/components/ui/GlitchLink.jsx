...
const GlitchLink = ({ to, text, kanji }) => {
    ...
    return (
        ...
        >
            <span
                className="glitch-wrapper relative inline-block text-center"
            >
                {/* English Text (Maintains Width) */}
                <span
            ...
            {/* Active underline */}
            <span className={`absolute bottom-0 left-0 w-full h-0.5 bg-prakida-water transform transition-transform duration-300 origin-left ${isActive ? 'scale-x-100' : 'scale-x-0 group-hover:scale-x-100'}`}></span>
        </Link>
    );
};
