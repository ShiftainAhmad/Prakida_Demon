            ...
            <div className="relative flex overflow-x-hidden group">
                ...
                <div className="animate-marquee whitespace-nowrap flex gap-16 py-4">
                    ...
                    {[...sponsors, ...sponsors].map((sponsor, idx) => (
                        <div key={idx} className="text-2xl md:text-4xl font-display font-bold text-white/20 hover:text-prakida-flame transition-colors duration-300 uppercase select-none">
                            {sponsor}
                        </div>
                    ))}
                    {[...sponsors, ...sponsors].map((sponsor, idx) => (
                        <div key={`dup-${idx}`} className="text-2xl md:text-4xl font-display font-bold text-white/20 hover:text-prakida-flame transition-colors duration-300 uppercase select-none">
                            {sponsor}
                        </div>
                    ))}
                </div>
